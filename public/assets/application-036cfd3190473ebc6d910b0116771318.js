...
(function() {
  $(function() {
    ...
    return jqtouch = $.jQTouch({});
  });

}).call(this);
(function() {
  var close_places, close_places_loading, end_close_places, end_just_eaten, filter_keyword_gv, getDocHeight, initialize_map, just_eaten_loading, last_page, latitude, latitude_user, load_more_listener_off, load_more_listener_on, load_more_photos_when_page_bottom_reached, loading, longitude, longitude_user, main, map, marker, page_close_places, page_just_eaten, popular_places, radius_gv, refresh_listeners, show_map_listener, show_place, show_place_listener, spinner_off, spinner_on;
  ...
  show_map_listener = function() {
    $("button.show_map").off("click", function() {});
    return $("button.show_map").on("click", function() {
  ...
  show_place_listener = function() {
    $("button.show_place").off('click');
    return $("button.show_place").on('click', function() {
      var id;
  ...
  refresh_listeners = function() {
    show_map_listener();
    return show_place_listener();
  };

  spinner_on = function(spinner_id) {
    return $(spinner_id)[0].style.display = "inline";
  };

  spinner_off = function(spinner_id) {
    return $(spinner_id)[0].style.display = "none";
  };
  ...
  load_more_photos_when_page_bottom_reached = function() {
    if ($(window).height() + $(window).scrollTop() - getDocHeight() > -200) {
      ...
      if (window.location.hash === "#close_places" && radius_gv !== -1 && !end_close_places) {
        return close_places_loading(radius_gv, filter_keyword_gv);
      }
    }
  };

  load_more_listener_on = function() {
    $(window).bind("scroll", load_more_photos_when_page_bottom_reached);
    return loading = false;
  };

  load_more_listener_off = function() {
    loading = true;
    return $(window).unbind("scroll", load_more_photos_when_page_bottom_reached);
  };

  close_places_loading = function(radius, filter_keyword) {
    return $.ajax({
      ...
      success: function(ret) {
        var center, content, i, image, imgs_list, j, li, link2, name, vicinity, _i, _j, _ref, _ref1;
        console.log(ret);
        if (ret.length === 0) {
          ...
          return end_close_places = true;
        } else {
          for (i = _i = 0, _ref = ret.length - 1; 0 <= _ref ? _i <= _ref : _i >= _ref; i = 0 <= _ref ? ++_i : --_i) {
            li = $("<li></li>");
            ...
            for (j = _j = 0, _ref1 = ret[i]['photos_filtered'].length - 1; 0 <= _ref1 ? _j <= _ref1 : _j >= _ref1; j = 0 <= _ref1 ? ++_j : --_j) {
            ...
            if (ret[i]['reviews_count'] > 0) {
              $("#place_star_close_places" + ret[i]['id']).raty({
                width: 250,
                readOnly: true,
                score: ret[i]['rate_average']
              });
            }
          }
          page_close_places++;
          return show_map_listener();
        }
      },
      beforeSend: function() {
        load_more_listener_off();
        return spinner_on("#spinner_close_places");
      },
      complete: function() {
  ...
  just_eaten_loading = function() {
    return $.ajax({
      url: "/photos?page=" + page_just_eaten,
      dataType: "json",
      contentType: "application/json",
      success: function(ret) {
        var button, i, image, li, link2, minutes_ago, name, vicinity, _i, _ref;
        if (ret.length === 0) {
          return end_just_eaten = true;
        } else {
          for (i = _i = 0, _ref = ret.length - 1; 0 <= _ref ? _i <= _ref : _i >= _ref; i = 0 <= _ref ? ++_i : --_i) {
            li = $("<li></li>");
            ...
            if (ret[i]['place']['reviews_count'] > 0) {
              $("#place_star_just_eaten" + ret[i]['id'] + ret[i]['place']['id']).raty({
                width: 250,
                readOnly: true,
                score: ret[i]['place']['rate_average']
              });
            }
          }
          return page_just_eaten++;
        }
      },
      beforeSend: function() {
        load_more_listener_off();
        return spinner_on("#spinner_just_eaten");
      },
      complete: function() {
        spinner_off("#spinner_just_eaten");
        refresh_listeners();
        return load_more_listener_on();
      }
    });
  };

  close_places = function(radius, filter_keyword) {
    ...
    if (navigator.geolocation) {
      return navigator.geolocation.getCurrentPosition(function(position) {
        ...
        return close_places_loading(radius, filter_keyword);
      });
    } else {
      return alert("Impossible to find your location");
    }
  };
  ...
  show_place = function(id) {
    ...
    return $.ajax({
      ...
      contentType: "application/json",
      success: function(ret) {
        var author, body_review, center, content, div_star, image, imgs_list, j, li, link2, name, review, review_div, reviews, reviews_div, vicinity, _i, _j, _len, _ref;
        if (ret.length === 0) {
          li = $("<li><center><p>No results found</p></center></li>");
          return $("ul.edgetoedge#show_place").append(li);
        } else {
          li = $("<li></li>");
          ...
          imgs_list = $("<div class='imgs' data-current_img=0></div>");
          content.append(imgs_list);
          for (j = _i = 0, _ref = ret['photos'].length - 1; 0 <= _ref ? _i <= _ref : _i >= _ref; j = 0 <= _ref ? ++_i : --_i) {
          ...
          if (ret['reviews_count'] > 0) {
            $("#place_star_show_place" + ret['id']).raty({
              width: 250,
              readOnly: true,
              score: ret['rate_average']
            });
          }
          reviews_div = $(".reviews");
          reviews = ret['reviews'];
          for (_j = 0, _len = reviews.length; _j < _len; _j++) {
            review = reviews[_j];
          ...
          if (reviews.length === 0) {
            return $(".info.view.empty")[0].style.display = "block";
          } else {
            return $(".info.view.empty")[0].style.display = "none";
          }
        }
      },
      beforeSend: function() {
        ...
        return spinner_on("#spinner_show_place");
      },
      complete: function() {
        var form_review;
        if ((form_review = $(".form_review")).length === 1) {
          form_review[0].style.display = "inline";
        }
        show_map_listener();
        spinner_off("#spinner_show_place");
        return listen_to_swipe();
      }
    });
  };

  popular_places = function() {
    if ($("ul.edgetoedge#popular_places").find("li").length === 0) {
      return $.ajax({
        url: "/places?page=popular",
        dataType: "json",
        contentType: "application/json",
        success: function(ret) {
          var button, col1, col2, i, li, name, row, _i, _ref, _results;
          _results = [];
          for (i = _i = 0, _ref = ret.length - 1; 0 <= _ref ? _i <= _ref : _i >= _ref; i = 0 <= _ref ? ++_i : --_i) {
            li = $("<li></li>");
            ...
            row.append(col2);
            li.append(row);
            _results.push($("ul.edgetoedge#popular_places").append(li));
          }
          return _results;
        },
        beforeSend: function() {
          return spinner_on("#spinner_popular_places");
        },
        complete: function() {
  ...
  main = function() {
    ...
    if (location_page === "home") {
      close_places('1.0', '');
      listen_to_swipe();
      $("#search").on("click", function(event) {
        var filter_keyword, radius;
        ...
        radius_gv = radius;
        return filter_keyword_gv = filter_keyword;
      });
      $("#myModal").on("shown.bs.modal", function() {
        google.maps.event.trigger(map, "resize");
        return map.setCenter(new google.maps.LatLng(latitude, longitude));
      });
      return $(window).on('hashchange', function() {
        var anchor;
        anchor = window.location.hash;
        if (anchor === "#just_eaten") {
        ...
        if (anchor === "#popular_places") {
          popular_places();
        }
        return last_page = anchor;
      });
    }
  };
...
(function() {
  var listen_submit_review;

  listen_submit_review = function(event) {
    var form;
    event.preventDefault();
    ...
    return $.ajax({
      ...
      success: function(review) {
        var author, body_review, div_star, review_div, reviews_div;
        if (review === "You already have posted a review for this place.") {
          return alert("You already have posted a review for this place.");
        } else {
          reviews_div = $(".reviews");
          review_div = $("<div class='info view' style='text-align: left;'></div>");
          author = $("<p><img src=" + review['user']['image'] + "> " + review['user']['name'] + " (" + review['created_at'] + ")</p>");
      ...
      beforeSend: function() {
        ...
        return $("#submit_review").removeAttr("href").text("").append($("<img src=/assets/spiffygif.gif >"));
      },
      complete: function() {
        ...
        return $("#submit_review").on("click", function(event) {
          return listen_submit_review(event);
        });
      },
      data: form.serialize()
    });
