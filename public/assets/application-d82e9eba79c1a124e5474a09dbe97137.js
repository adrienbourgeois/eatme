...
(function() {
  var end, first_time, getDocHeight, get_close_places, get_location, init, initialize, just_eaten, latitude, latitude_user, load_more_listener_off, load_more_listener_on, load_more_photos, load_more_photos_when_page_bottom_reached, longitude, longitude_user, map, marker, page, show_map_listener;
    ...
    return $(window).on('hashchange', function() {
      ...
      if (anchor === "#popular_places") {
        ...
        return $.ajax({
          ...
          contentType: "application/json",
          success: function(ret) {
            var button, i, li, name, _i, _ref, _results;
            _results = [];
            for (i = _i = 0, _ref = ret.length - 1; 0 <= _ref ? _i <= _ref : _i >= _ref; i = 0 <= _ref ? ++_i : --_i) {
              li = $("<li></li>");
              name = $("<span>" + ret[i]['name'] + " (" + ret[i]['photos'].length + ")</span>");
              button = $("<button type='button' class='btn btn-default btn-lg' data-place_id=3><span class='.glyphicon .glyphicon-arrow-right'></span></button>");
              li.append(name);
              li.append(button);
